class SinglyLinkedList
  include Enumerable    #Mixin Ruby will give you all free methods after 'each' method created - cmd: bros.methods
  ...
  def append(value)
    ...
    # while last_node && last_node.next != nil
    #   last_node = last_node.next
    # end
  ...
  def length    #Also #count and #size (Ruby has nice way to alias things)
    tally = 0
    node = @head
  ...
  def find(needle)
    # Returns the node container the needles as its value
    node = @head
    until node.nil?
